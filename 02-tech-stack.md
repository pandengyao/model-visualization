...
| **UI 样式** | Tailwind CSS | `tailwindcss ^3.4` **(锁 v3，不上 v4)** | v4 生态第三方 UI 库适配未完成，v1.0 稳定优先。**升级触发条件**：Tailwind v4 + shadcn/ui 适配 ≥ 1.0 正式版 + @react-three/drei 兼容验证通过 |
...
> - `tailwindcss` 锁 `^3.4`，**禁止**升级到 v4（生态未就绪；2026-04 核验，v1.1 启动前重审）。
...
- **Hover**：scale（1.0 → 1.02）+ 辉光（emissive intensity 提升）
...
| **MoE 专家批渲染** | Drei `<Instances>` / `<InstancedMesh>` | 单 draw call 渲染 256 专家 |
...
> - **相机过渡**：Drei `<OrbitControls>` 的内置 damping 平滑插值
>
...
| **Schema + Provenance** | Pydantic v2 | `>=2.6` | 所有 API I/O 类型强制，`Provenance` 可追溯（对齐原则 11） |
...
| **JSON 序列化** | **orjson** | `>=3.10` | **交互响应硬约束**（原则 7 例外）：大结构树序列化 5–10× 快于 stdlib |
...
| **热更新推送** | WebSocket（PATCH 热更新推送通道，详见 04 §4.7）；SSE 用于冷启动分段推送 | — | `PATCH /config` 后向前端推送增量，驱动 3D 场景即时重渲；契约见 04 / 11 §8 |

> **CPU-only + meta-device**：后端不需要 GPU，`torch.device("meta")` 仅构造结构不分配权重内存，Docker 镜像不含 CUDA（控制体积 ~1.3–2.0 GB）。

### 2.6 扩展点依赖（对齐原则 8：v1.0 架构最健壮可扩展）

v1.0 必须具备以下插件化扩展点，但**扩展机制本身拒绝过度工程**：

| 扩展点 | 注册机制 | 明确禁止 |
|---|---|---|
| **Adapter**（模型架构检测） | Python `Protocol` + `@register` 装饰器 + `__init__.py` 显式 import 触发注册（对齐 11 §1.2） | `setuptools entry_points`、`pluggy`、任何自动发现机制 |
...
| **AnimationLayer** | v1.0 仅实现 L1/L2 具体组件，不抽象 Protocol（见 11 §3） | 同上 |
| **MemoryEstimator** | 显式注册表 | 同上 |

> v1.2+ 规划中的 `ParallelismStrategy` 不在 v1.0 扩展点表范围；其接口形态在 v1.2 启动前独立 ADR 评审时定稿（见 11 §4）。

> **设计原则**：注册表必须是**可 `grep` 出来的显式代码**。v1.0 Adapter/Template 各 4 个（LLaMA / LLaMA-MoE / DeepSeek-MoE / Generic），隐式插件机制带来的复杂度远大于收益。
...
### 2.8 测试 / Lint 工具链（对齐原则 3/7/11）

**前端**：
- **Vitest**：单元 / 集成测试；覆盖 Adapter/Template/Registry 等纯函数与 React hooks。`vitest run --coverage` 在 CI 强制 ≥ 80%（v1.0 DoD，对齐 06 Phase 1/2 DoD）。Phase 1 起强制；Phase 0 仅记录（对齐 06 Phase 0 DoD）
- **@testing-library/react**：组件测试（Provenance 徽标、Config 编辑器、GPU 选择器）
- **Playwright**：E2E + 视觉回归；承载 07 验证矩阵（v1.0 为 5 个代表模型）与 05 §5.4 视觉规范验证（v1.0 为 8 张快照）
...
**CI 关卡（Phase 0/1 分层）**：
```
Phase 0：前端 pnpm lint && pnpm typecheck && pnpm test:unit
         后端 ruff check . && ruff format --check . && mypy src/ && pytest --cov=src
Phase 1 起：在 Phase 0 基础上追加 pnpm test:e2e
```
