...
class ModuleNode(BaseModel):
    ...
    # [ADDED 2026-04-25] 对齐 04 §4.2.2：结构粒度（原则 6 预留）
    # v1.0 默认对所有 layer 级节点产出 "block"；"op" 级粒度保留给 v1.1+ 动画/张量流展开
    level: Literal["layer", "block", "op", "tensor"]
    params: int               # 精确参数量
...
class ArchitectureProfile(BaseModel):
    ...
    # 无匹配 Adapter 时回退 "G"（非静默回退 A，对齐原则 8）
...
# v1.0 推理版只填 weights / kv_cache / activations；训练版（gradients /
# optimizer_states / comm_buffer）字段预留给 v1.1，v1.0 置 None。
class MemoryBreakdown(BaseModel):
    # v1.0 必填
    weights_bytes: int
    kv_cache_bytes: int
    activations_bytes: int
    # v1.1 预留（v1.0 均为 None）
